...
function SortingDirectJumpAggregation(nj::Int, njt::T, et::T, crs::Vector{T}, sr::T,
    ...
    if dep_graph === nothing
        if (get_num_majumps(maj) == 0) || !isempty(rs)
            error(
                "To use ConstantRateJumps with the SortingDirect algorithm a dependency graph must be supplied.",
            )
        else
...
function aggregate(aggregator::SortingDirect, u, p, t, end_time,
        constant_jumps,
