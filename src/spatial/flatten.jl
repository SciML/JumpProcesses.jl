...
"""
function flatten(
    netstoch::AbstractArray,
    reactstoch::AbstractArray,
    rx_rates::AbstractArray,
    spatial_system,
    u0::Matrix{Int},
    tspan,
    ...
    for ci in CartesianIndices(hop_constants)
        ...
        hop_constants[ci] =
            hopping_constants[species, site] * ones(outdegree(spatial_system, site))
    end
    flatten(
        netstoch,
        reactstoch,
...
function flatten(
    ...
    sum_outdegrees = sum(outdegree(spatial_system, site) for site = 1:num_nodes)
