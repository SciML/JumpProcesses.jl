...
@fastmath function time_to_next_jump(
    p::DirectJumpAggregation{T,S,F1,F2,RNG},
    u,
    params,
    t,
    ...
    if !isempty(rates)
        ...
        @inbounds for i = idx:length(cur_rates)
...
@fastmath function time_to_next_jump(
    ...
    idx += 1
    rates = p.rates
    @inbounds for i = 1:length(p.rates)
