function isinplace_jump(p, rj)
    if p isa DiscreteProblem &&
       p.f === DiffEqBase.DISCRETE_INPLACE_DEFAULT &&
       rj !== nothing
...
"""
mutable struct JumpProblem{iip,P,A,C,J<:Union{Nothing,AbstractJumpAggregator},J2,J3,J4,R} <:
...
function DiffEqBase.remake(thing::JumpProblem; kwargs...)
    ...
    if :prob ∉ keys(kwargs)
        ...
        any(k -> k in keys(kwargs), (:u0, :p, :tspan)) && error(
            "If remaking a JumpProblem you can not pass both prob and any of u0, p, or tspan.",
        )
        dprob = kwargs[:prob]
...
JumpProblem(prob, jumps::ConstantRateJump; kwargs...) =
    JumpProblem(prob, JumpSet(jumps); kwargs...)
JumpProblem(prob, jumps::VariableRateJump; kwargs...) =
    JumpProblem(prob, JumpSet(jumps); kwargs...)
JumpProblem(prob, jumps::RegularJump; kwargs...) =
    JumpProblem(prob, JumpSet(jumps); kwargs...)
JumpProblem(prob, jumps::MassActionJump; kwargs...) =
    JumpProblem(prob, JumpSet(jumps); kwargs...)
JumpProblem(prob, jumps::AbstractJump...; kwargs...) =
    JumpProblem(prob, JumpSet(jumps...); kwargs...)
...
JumpProblem(prob, aggregator::AbstractAggregatorAlgorithm, jumps::RegularJump; kwargs...) =
    JumpProblem(prob, aggregator, JumpSet(jumps); kwargs...)
JumpProblem(
    prob,
    aggregator::AbstractAggregatorAlgorithm,
    jumps::AbstractMassActionJump;
    kwargs...,
) = JumpProblem(prob, aggregator, JumpSet(jumps); kwargs...)
JumpProblem(
    prob,
...
JumpProblem(prob, jumps::JumpSet; kwargs...) =
...
function JumpProblem(
    ...
    aggregator::AbstractAggregatorAlgorithm,
    jumps::JumpSet;
    save_positions = typeof(prob) <: DiffEqBase.AbstractDiscreteProblem ? (false, true) :
                     (true, true),
    rng = DEFAULT_RNG,
    scale_rates = true,
    useiszero = true,
    ...
    if typeof(jumps.variable_jumps) <: Tuple{}
        ...
        variable_jump_callback =
            build_variable_callback(CallbackSet(), 0, jumps.variable_jumps...; rng = rng)
    end
...
function extend_problem(prob::DiffEqBase.AbstractDiscreteProblem, jumps; rng = DEFAULT_RNG)
    error(
        "VariableRateJumps require a continuous problem, like an ODE/SDE/DDE/DAE problem.",
    )
end
...
function extend_problem(prob::DiffEqBase.AbstractODEProblem, jumps; rng = DEFAULT_RNG)
    ...
    u0 = ExtendedJumpArray(
        prob.u0,
        [-randexp(rng, ttype) for i = 1:length(jumps.variable_jumps)],
    )
...
Base.summary(io::IO, prob::JumpProblem) = string(
