...
@test maximum([sol.u[i][2] for i in 1:length(sol.u)]) <= 1.0e-12
@test maximum([sol.u[i][3] for i in 1:length(sol.u)]) <= 1.0e-12
...
let
    ...
    birth_jump = VariableRateJump(
        birth_rate, birth_affect!; save_positions = (
            false, false,
        )
    )
    ...
    death_jump = VariableRateJump(
        death_rate, death_affect!; save_positions = (
            false, false,
        )
    )
...
let
    ...
    function switch_affect!(integ)
        ...
        return reset_aggregated_jumps!(integ)
    end
    ...
    @test maximum(abs, sol.u[end].jump_u) < 1.0e6
