...
SplitCoupledJumpProblem(
    ...
    cat_problems(prob.prob, prob_control.prob),
    aggregator,
    build_split_jumps(prob, prob_control, coupling_map)...;
    kwargs...,
)
...
function cat_problems(prob::DiscreteProblem, prob_control::DiffEqBase.AbstractODEProblem)
    ...
    if !(typeof(prob.f) <: typeof(DiffEqBase.DISCRETE_INPLACE_DEFAULT))
        @warn(
            "Coupling to DiscreteProblem with nontrivial f. Note that, unless scale_by_time=true, the meaning of f will change when using an ODE/SDE/DDE/DAE solver."
        )
    end
    new_f = function (du, u, p, t)
        prob.f(@view(du[1:l]), u.u, p, t)
...
cat_problems(prob_control::DiffEqBase.AbstractODEProblem, prob::DiscreteProblem) =
    cat_problems(prob, prob_control)
cat_problems(prob_control::DiscreteProblem, prob::DiffEqBase.AbstractSDEProblem) =
    cat_problems(prob, prob_control)
cat_problems(
    prob_control::DiffEqBase.AbstractODEProblem,
    prob::DiffEqBase.AbstractSDEProblem,
