...
@time sol = solve(coupled_prob, FunctionMap(); rng)
@time solve(jump_prob, FunctionMap(); rng)
...
coupled_prob = SplitCoupledJumpProblem(
    jump_prob, jump_prob_control, Direct(), coupling_map)
sol = solve(coupled_prob, Tsit5(); rng)
...
coupled_prob = SplitCoupledJumpProblem(
    jump_prob, jump_prob_control, Direct(), coupling_map)
sol = solve(coupled_prob, SRIW1(); rng)
@test mean([abs(s[1] - s[2]) for s in sol.u]) <= 5.0
...
for i in 1:Nsims
    global Amean
    local sol = solve(jump_prob, Tsit5(); saveat = 10.0, rng)
    Amean += sol[1, end]
end
Amean /= Nsims
