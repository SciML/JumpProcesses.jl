...
function RSSACRJumpAggregation(nj::Int, njt::F, et::F, crs::Vector{F}, sum_rate::F, maj::S,
    ...
    if vartojumps_map === nothing
        if (get_num_majumps(maj) == 0) || !isempty(rs)
            error(
                "To use the RSSACR algorithm a map from variables to dependent jumps must be supplied.",
            )
        else
    ...
    if jumptovars_map === nothing
        if (get_num_majumps(maj) == 0) || !isempty(rs)
            error(
                "To use the RSSACR algorithm a map from jumps to dependent variables must be supplied.",
            )
        else
...
function generate_jumps!(p::RSSACRJumpAggregation, integrator, u, params, t)
    ...
    rerl += randexp(rng)
    while rejectrx(ma_jumps, num_majumps, rates, cur_rate_high,
        cur_rate_low, rng, u, jidx,
        params, t)
...
"""
@inline function update_dependent_rates!(p::RSSACRJumpAggregation,
        u::AbstractVector,
        params, t)
