...
# GPU-compatible Poisson sampling with LCG
@inline function poisson_sample(rate::Float64, seed::UInt64, idx::Int64)
    L = exp(-rate)
    k = 0
    p = 1.0
    seed = seed ⊻ UInt64(idx)  # Mix index into seed
    while p > L
        k += 1
...
@kernel function simple_tau_leaping_kernel(@Const(probs_data), _us, _ts, dt, @Const(rj_data),
    ...
    for j in 2:n
        ...
        # Poisson sampling using LCG
        @inbounds for k in 1:num_jumps
            counts[k] = Float64(poisson_sample(rate_cache[k], seed + UInt64(i * n + j * k), i))
        end
