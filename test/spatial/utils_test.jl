...
rel_tol = 0.01
grids = [
    DiffEqJump.CartesianGridRej(dims),
    DiffEqJump.CartesianGridIter(dims),
    Graphs.grid(dims),
]
for grid in grids
...
for site = 1:num_nodes
    DiffEqJump.update_rx_rates!(rx_rates, 1:num_rxs, u, site)
    rx_props = [DiffEqJump.evalrxrate(u[:, site], rx, ma_jumps) for rx = 1:num_rxs]
...
spec_probs = ones(num_species) / num_species
hop_rates_structs = [
    DiffEqJump.HopRatesGraphDsij(hop_constants),
    DiffEqJump.HopRatesGridDsij(hop_constants, g),
]
...
spec_probs = ones(num_species) / num_species
hop_rates_structs = [
    DiffEqJump.HopRatesGraphDsLij(species_hop_constants, site_hop_constants),
    DiffEqJump.HopRatesGridDsLij(species_hop_constants, site_hop_constants, g),
]
...
spec_probs = ones(num_species) / num_species
hop_rates_structs = [
    DiffEqJump.HopRatesGraphDsiLij(species_hop_constants, site_hop_constants),
    DiffEqJump.HopRatesGridDsiLij(species_hop_constants, site_hop_constants, g),
]
