...
function mastereqmean(u, rates)
    ...
    β = u[2]
    γ = u[3]
    d₊ = [rates[1] * (a + 1) * (β - α + a + 1) for a = 0:(α-1)]
    d₋ = [rates[2] * (γ + α - a + 1) for a = 1:α]
    d = [-rates[1] * a * (β - α + a) - rates[2] * (γ + α - a) for a = 0:α]
    L = diagm(-1 => d₋, 0 => d, 1 => d₊)
    P_a = nullspace(L)
    P_a ./= sum(P_a)
    P_a .= abs.(P_a)
...
for alg in algs
    local jprob =
        JumpProblem(prob, alg, majumps, save_positions = (false, false), rng = rng)
