...
for i in 1:length(update_nums)
    ...
    pq = pq_setup(rates)
    elapsed_time = 0.0
    update!(pq, 1, randexp() / rates[1]) #force compilation
    for j in 1:samples
        n = rand(1:num_rates); rates[n] = rand()
        elapsed_time += @elapsed update!(pq, n, randexp() / rates[n]) 
    end
...
plot!(plt_updates, xaxis = :log, xticks = (update_nums, xticks))
