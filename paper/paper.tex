...
\end{equation}
The idea is to draw realizations from the unit rate Exponential process and solve Equation~\ref{eqn:inverse} for \( t_n \)  to determine the next event/firing time. We illustrate this in Algorithm~\ref{algo:sim-inverse} where we adapt Algorithm 7.4~\cite{daley2003}. \comment{G: \st{The advantage of using the inverse method is that it is easier to couple the point process with differential equations and solve the system with the same numerical integrator.}} \comment{G: I have removed the previous bit, I am not sure if this is really an advantage seeing how this method still needs to root find and so it is inefficient.}
...
\begin{algorithm}[h]
\begin{algorithmic}[1]
  \Procedure{NextQueueTime}{\( t \), \( \lambda^\ast \), \( H_{t} \),}
...
\begin{algorithmic}[1]
  \Procedure{QueueingMethod}{\( [0, T) \), \( \{ \lambda_{k}^\ast \} \), \( \{ f_{k}^\ast \} \),}
    ...
    \For{i=1,M} \label{line:queueing-init-begin}
      \State \( (t_i, \bar{B}_i^\ast, \ubar{B}_i^\ast, a_i) \leftarrow \operatorname{NextQueueTime}(0, H_{T^-}, \lambda_{i}^\ast(\cdot)) \) \label{line:candidate-one}
    ...
    \While{\( t < T \)}
      ...
      \If{ \( a_i \)}
        ...
        \For{\( j \in \{ i \} \cup \operatorname{Neighborhood}(i) \)}
          \State \( (t_j, \bar{B}_j^\ast, \ubar{B}_j^\ast, a_j) \leftarrow \operatorname{NextQueueTime}(0, H_{T^-}, \lambda_{j}^\ast(\cdot)) \)
