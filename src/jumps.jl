...
RegularJump(rate, c, numjumps::Int; kwargs...) =
...
struct MassActionJump{T,S,U,V} <: AbstractMassActionJump
    ...
    function MassActionJump{Nothing,Vector{S},Vector{U},V}(
        ::Nothing,
        rs_in::Vector{S},
        ns::Vector{U},
        pmapper::V,
        ...
        useiszero::Bool,
        nocopy::Bool,
    ) where {S<:AbstractVector,U<:AbstractVector,V}
        rs = nocopy ? rs_in : copy(rs_in)
        for i in eachindex(rs)
            if useiszero && (length(rs[i]) == 1) && iszero(rs[i][1][1])
                rs[i] = typeof(rs[i])()
...
MassActionJump(
    ...
    useiszero = true,
    nocopy = false,
) where {T<:AbstractVector,S,U} = MassActionJump(
    usr,
    rs,
    ns,
    nothing;
...
MassActionJump(
    ...
    useiszero = true,
    nocopy = false,
) where {T<:Number,S,U} = MassActionJump(
    usr,
    rs,
    ns,
    nothing;
...
function update_parameters!(maj::MassActionJump, newparams; scale_rates = true, kwargs...)
    (maj.param_mapper === nothing) && error(
...
JumpSet(vj, cj, rj, maj::MassActionJump{S,T,U,V}) where {S<:Number,T,U,V} =
...
function JumpSet(;
    variable_jumps = (),
    constant_jumps = (),
    regular_jumps = nothing,
    massaction_jumps = nothing,
)
...
@inline split_jumps(vj, cj, rj, maj, v::VariableRateJump, args...) =
    split_jumps((vj..., v), cj, rj, maj, args...)
@inline split_jumps(vj, cj, rj, maj, c::ConstantRateJump, args...) =
    split_jumps(vj, (cj..., c), rj, maj, args...)
@inline split_jumps(vj, cj, rj, maj, c::RegularJump, args...) =
    split_jumps(vj, cj, regular_jump_combine(rj, c), maj, args...)
@inline split_jumps(vj, cj, rj, maj, c::MassActionJump, args...) =
    split_jumps(vj, cj, rj, massaction_jump_combine(maj, c), args...)
@inline split_jumps(vj, cj, rj, maj, j::JumpSet, args...) = split_jumps(
    (vj..., j.variable_jumps...),
    (cj..., j.constant_jumps...),
    regular_jump_combine(rj, j.regular_jump),
    massaction_jump_combine(maj, j.massaction_jump),
    args...,
)
...
regular_jump_combine(rj1::RegularJump, rj2::RegularJump) =
    error("Only one regular jump is allowed in a JumpSet")


# functionality to merge two mass action jumps together
check_majump_type(maj::MassActionJump{S,T,U,V}) where {S<:Number,T,U,V} =
    setup_majump_to_merge(
        maj.scaled_rates,
        maj.reactant_stoch,
        maj.net_stoch,
        maj.param_mapper,
    )
check_majump_type(maj::MassActionJump{Nothing,T,U,V}) where {T,U,V} = setup_majump_to_merge(
    maj.scaled_rates,
    maj.reactant_stoch,
    maj.net_stoch,
    maj.param_mapper,
...
function setup_majump_to_merge(
    ...
    ns::AbstractVector{U},
    pmapper,
) where {T<:AbstractVector,S<:AbstractArray,U<:AbstractArray}
...
function setup_majump_to_merge(
    ...
    ns::U,
    pmapper,
) where {S<:Number,T<:AbstractArray,U<:AbstractArray}
    MassActionJump(
        [sr],
        [rs],
        [ns],
...
function majump_merge!(
    ...
    ns::AbstractVector{W},
    param_mapper,
) where {U<:Union{AbstractVector,Nothing},V<:AbstractVector,W<:AbstractVector,X}
...
function majump_merge!(
    ...
    ns::U,
    param_mapper::V,
) where {T<:Union{Number,Nothing},S<:AbstractArray{<:Pair},U<:AbstractArray{<:Pair},V}
...
massaction_jump_combine(maj1::MassActionJump, maj2::MassActionJump) = majump_merge!(
    maj1,
    maj2.scaled_rates,
    maj2.reactant_stoch,
    maj2.net_stoch,
...
function get_jump_info_fwrappers(u, p, t, constant_jumps)
    RateWrapper =
        FunctionWrappers.FunctionWrapper{typeof(t),Tuple{typeof(u),typeof(p),typeof(t)}}
