...
diffusivity = 0.1
dim = 2
dims = Tuple([linear_size for i = 1:dim])
num_nodes = prod(dims)
...
for alg in algs
    push!(
        ...
        JumpProblem(
            ...
            alg,
            majumps,
            hopping_constants = hop_constants,
            spatial_system = grids[1],
            save_positions = (false, false),
        ),
    )
...
for ci in CartesianIndices(hop_constants)
    ...
    hop_constants[ci] =
        repeat([hopping_constants[species, site]], outdegree(grids[1], site))
end
for alg in algs
    push!(
        jump_problems,
