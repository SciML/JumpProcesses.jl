...
# Reset the random number generator for discrete callbacks
...
# Reset integrated intensities for VariableRateJumps
if !isempty(jump_prob.variable_jumps)
    if jump_prob.prob.u0 isa ExtendedJumpArray
        randexp!(_jump_prob.rng, jump_prob.prob.u0.jump_u)
        jump_prob.prob.u0.jump_u .*= -1
    else
        @warn "Skipping reset of integrated intensities because u0 is not an ExtendedJumpArray."
    end
end
