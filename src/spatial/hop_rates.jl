...
function HopRates(
        ...
        spatial_system,
) where {F <: Number, SpecHop <: Vector{F},
...
function HopRates(
        p::Pair{SpecHop, SiteHop},
        spatial_system,
) where {F <: Number, SpecHop <: Matrix{F},
        SiteHop <: Vector{Vector{F}}}
...
"""
function update_hop_rates!(hop_rates::AbstractHopRates,
        species::AbstractArray, u, site,
        spatial_system)
...
function update_hop_rate!(hop_rates::AbstractHopRates, species, u, site, spatial_system)
    ...
    @inbounds rates[species, site] = evalhoprate(hop_rates, u, species,
        site,
        spatial_system)
...
function sample_target_site(hop_rates::HopRatesGraphDsij, site,
        species, rng,
        spatial_system)
...
function sample_target_site(hop_rates::HopRatesGraphDsLij,
        site, species, rng,
        spatial_system)
...
function sample_target_site(hop_rates::HopRatesGraphDsiLij,
        site, species, rng,
        spatial_system)
    @inbounds cum_hop_consts = hop_rates.hop_const_cumulative_sums[site]
    @inbounds n = searchsortedfirst(cum_hop_consts, rand(rng) * cum_hop_consts[end])
...
function HopRatesGridDsiLij(species_hop_constants::Matrix{F}, site_hop_constants::Matrix{F};
        do_cumsum = true) where {F <: Number}
