...
function DiffEqBase.__init(
        ...
        alg::DiffEqBase.AbstractDEAlgorithm; merge_callbacks = true, kwargs...
    ) where {P}
    # Merge jump_prob.kwargs with passed kwargs
...
function reset_jump_problem!(jump_prob, seed)
    return if seed !== nothing && !isempty(jump_prob.jump_callback.discrete_callbacks)
        Random.seed!(
            jump_prob.jump_callback.discrete_callbacks[1].condition.rng,
