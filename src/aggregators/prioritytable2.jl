...
mutable struct PriorityTable2{F}
...
function PriorityTable2(minpriority)
    ...
    PriorityTable2{F}(minpower, gids, gnums, gsums, gsum, pidtogroup)
end

function PriorityTable2(minpriority, priorities)
    pt = PriorityTable2(minpriority)
...
function sample(pt::PriorityTable2{F}, priorities, rng = rng=Random.GLOBAL_RNG) where F
    ...
    maxpriority = 2^(pt.minpower + gid-2)
    while true
...
"pad table to newgid"
function padtable!(pt::PriorityTable2{F}, newgid) where F
...
function insert!(pt::PriorityTable2, pid, gid::Int)
...
function update!(pt::PriorityTable2{F}, pid, oldpriority, newpriority) where F
...
function reset!(pt::PriorityTable2{F}) where F
    pt.gnums .= 0
    pt.gsums .= zero(F)
    pt.gsum = 0
    pt.pidtogroup .= (0,0)
...
function insert!(pt::PriorityTable2, pid, priority)
...
function Base.show(io::IO, pt::PriorityTable2)
...
function numgroups(pt::PriorityTable2)
    @assert length(pt.gids) == length(pt.gnums) == length(pt.gsums)
    length(pt.gsums)
end

function numpriorities(pt::PriorityTable2)
    length(pt.pidtogroup)
end

function groupsum(pt::PriorityTable2)
    pt.gsum
end