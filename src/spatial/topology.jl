...
"""
function pad_hop_vec!(
    to_pad::AbstractVector{F},
    grid,
    site,
    hop_vec::Vector{F},
) where {F<:Number}
...
function CartesianGridRej(dims::Tuple)
    ...
    nums_neighbors = [count(x -> x + CI[site] in CI, offsets) for site = 1:prod(dims)]
...
CartesianGridRej(dimension, linear_size::Int) =
    CartesianGridRej([linear_size for i = 1:dimension])
...
CartesianGridIter(dimension, linear_size::Int) =
    CartesianGridIter([linear_size for i = 1:dimension])
