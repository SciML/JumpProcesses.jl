...
@inline @fastmath function evalrxrate(
    speciesvec::AbstractVector{T},
    rxidx::S,
    majump::MassActionJump{U,V,W,X},
...
@inline @fastmath function executerx!(
    speciesvec::AbstractVector{T},
    rxidx::S,
    majump::M,
...
@inline @fastmath function executerx(
    ...
    @inbounds for specstoch in net_stoch
        speciesvec =
            setindex(speciesvec, speciesvec[specstoch[1]] + specstoch[2], specstoch[1])
    end
    speciesvec
...
) where {U,S,T,W<:Pair{S,T},V<:AbstractVector{W}}
...
function jump_to_vars_map(majumps)
    [[s for (s, c) in majumps.net_stoch[i]] for i = 1:get_num_majumps(majumps)]
end
