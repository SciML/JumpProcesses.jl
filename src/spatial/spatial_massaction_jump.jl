...
function SpatialMassActionJump(urates::A, srates::B, rs::S, ns::U, pmapper::V;
    ...
    SpatialMassActionJump{A, B, S, U, V}(urates, srates, rs, ns, pmapper,
        scale_rates,
        useiszero, nocopy)
end
...
function get_num_majumps(smaj::SpatialMassActionJump{Nothing, Nothing, S, U, V},
) where
        {S, U, V}
    0
end
function get_num_majumps(smaj::SpatialMassActionJump{Nothing, B, S, U, V},
) where
        {B, S, U, V}
    size(smaj.spatial_rates, 1)
end
function get_num_majumps(smaj::SpatialMassActionJump{A, Nothing, S, U, V},
) where
        {A, S, U, V}
    length(smaj.uniform_rates)
end
function get_num_majumps(smaj::SpatialMassActionJump{A, B, S, U, V},
) where
...
function evalrxrate(speciesmat::AbstractMatrix{T}, rxidx::S,
