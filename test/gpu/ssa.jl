...
# A fixed seed must reproduce, and different seeds must not collide.
let
    jump_prob = majump_prob([0.5], [[1 => 1]], [[1 => -1]], [1000], (0.0, 2.0))
    ensemble_prob = EnsembleProblem(jump_prob)

    solve_seeded(s) = solve(ensemble_prob, SSAStepper(), EnsembleGPUKernel(backend);
        trajectories = 100, saveat = 1.0, seed = s)

    a, b, c = solve_seeded(7), solve_seeded(7), solve_seeded(8)
    @test all(a.u[i].u == b.u[i].u for i in 1:100)
    @test any(a.u[i].u != c.u[i].u for i in 1:100)
end
...
# Unsupported inputs must be rejected rather than silently mis-simulated.
let
    rate(u, p, t) = 0.5u[1]
